...
def book_slot(db: Session, schedule_id: int, fernet: Fernet, prefilled_amenity_id: int | None = None):
    ...
    try:
        ...
        # Prepare API payload
        amenity_id = get_amenity_id(schedule.court_id or "1") if prefilled_amenity_id is None else prefilled_amenity_id
        payload = {
            "amenity_type_id": "10",
        ...
        logger.info(f"Booking slot {schedule_id}: Court {schedule.court_id} (amenity_id {amenity_id}) at {start_time_str} ({attempt_type} attempt)")
        ...
        if prefilled_amenity_id is None:
            logger.info(f"Booking {schedule_id} failed for court {schedule.court_id}, trying the other court")
            book_slot(db, schedule_id, fernet, prefilled_amenity_id=10 if schedule.court_id == "1" else 8)
            return
